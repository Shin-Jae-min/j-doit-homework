...
class AzureGrader:
    ...
    def recognize_simple(self, audio_file_path):
        """
        Performs simple STT to identify what was said.
        """
        try:
    ...
    def grade(self, audio_file_path, reference_text):
        """
        Evaluates the pronunciation against the reference text.
        """
        ...
        try:
            ...
            if result.reason == speechsdk.ResultReason.RecognizedSpeech:
                ...
                logger.error(f"Recognition canceled: {cancellation_details.reason}")
                return {"status": "error", "message": f"인식 취소됨: {cancellation_details.reason}"}
            else:
